...
static bool served_tai_is_found(mme_enb_t *enb)
{
    ...
    for (i = 0; i < enb->num_of_supported_ta_list; i++) {
        ...
        ogs_debug("###### num_of_supported_ta_list : %u \n", enb->num_of_supported_ta_list);
        ogs_debug("###### TRACKING AREA CODE : %u \n", enb->supported_ta_list[i].tac);
        ogs_debug("###### mcc: %u%u%u\n mnc: %u%u%u \n",
         enb->supported_ta_list[i].plmn_id.mcc1, enb->supported_ta_list[i].plmn_id.mcc2, enb->supported_ta_list[i].plmn_id.mcc3,
         enb->supported_ta_list[i].plmn_id.mnc1, enb->supported_ta_list[i].plmn_id.mnc2, enb->supported_ta_list[i].plmn_id.mnc3);

        ogs_debug("###### SERVED TAI INDEX %u \n", served_tai_index);

        if (served_tai_index >= 0 && served_tai_index < OGS_MAX_NUM_OF_SUPPORTED_TA) {
